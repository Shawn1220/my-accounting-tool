(function () {
  ...
  function renderDonut(el, items, key) {
    ...
    const circles = rows.map((row, i) => {
      const len = row[1] / total * c;
      const html = `<circle cx="60" cy="60" r="${r}" fill="none" stroke="${colors[i % colors.length]}" stroke-width="14" stroke-linecap="butt" stroke-dasharray="${len.toFixed(3)} ${(c-len).toFixed(3)}" stroke-dashoffset="${(-offset).toFixed(3)}" transform="rotate(-90 60 60)"/>`;
      offset += len;
      return html;
    }).join('');
    const legend = rows.map((row, i) => `<div class="legend-row"><i class="dot" style="background:${colors[i % colors.length]}"></i><span>${row[0]}</span><b>${money(row[1])}</b></div>`).join('');
    el.innerHTML = `<svg class="donut-svg" viewBox="0 0 120 120"><circle cx="60" cy="60" r="${r}" fill="none" stroke="rgba(130,140,160,.18)" stroke-width="14"/>${circles}<text class="donut-center-label" x="60" y="56" text-anchor="middle" fill="currentColor">支出</text><text class="donut-center-value" x="60" y="72" text-anchor="middle" fill="currentColor">${Math.round(total)}</text></svg><div class="legend">${legend}</div>`;
  }
